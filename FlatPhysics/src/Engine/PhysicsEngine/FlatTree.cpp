...
namespace FlatPhysics
{
	...
	void AABBTree::insertObject(const std::shared_ptr<FlatIAABB>& object)
	{
	...
	void AABBTree::removeObject(const std::shared_ptr<FlatIAABB>& object)
	{
	...
	void AABBTree::updateObject(const std::shared_ptr<FlatIAABB>& object)
	{
		unsigned nodeIndex = _objectNodeIndexMap[object];
		updateLeaf(nodeIndex, object->getAABB());
	}

	std::forward_list<std::shared_ptr<FlatIAABB>> AABBTree::queryOverlaps(const std::shared_ptr<FlatIAABB>& object) const
	{
		std::forward_list<std::shared_ptr<FlatIAABB>> overlaps;
		std::stack<unsigned> stack;
		FlatAABB testAabb = object->getAABB();
	...
	void AABBTree::insertLeaf(unsigned leafNodeIndex)
	{
		...
		while (!_nodes[treeNodeIndex].isLeaf())
		{
			...
			FlatAABB combinedAabb = treeNode.aabb.Merge(leafNode.aabb);

			float newParentNodeCost = 2.0f * combinedAabb.Area;
			float minimumPushDownCost = 2.0f * (combinedAabb.Area - treeNode.aabb.Area);
			...
			if (leftNode.isLeaf())
			{
				costLeft = leafNode.aabb.Merge(leftNode.aabb).Area + minimumPushDownCost;
			}
			else
			{
				FlatAABB newLeftAabb = leafNode.aabb.Merge(leftNode.aabb);
				costLeft = (newLeftAabb.Area - leftNode.aabb.Area) + minimumPushDownCost;
			}
			if (rightNode.isLeaf())
			{
				costRight = leafNode.aabb.Merge(rightNode.aabb).Area + minimumPushDownCost;
			}
			else
			{
				FlatAABB newRightAabb = leafNode.aabb.Merge(rightNode.aabb);
				costRight = (newRightAabb.Area - rightNode.aabb.Area) + minimumPushDownCost;
			}
	...
	void AABBTree::updateLeaf(unsigned leafNodeIndex, const FlatAABB& newAaab)
	{
