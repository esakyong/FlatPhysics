...
#define AABB_NULL_NODE 0xffffffff
	...
	{
		FlatAABB aabb;
		std::shared_ptr<FlatIAABB> object;
		// tree links
	...
	{
	private:
		std::map<std::shared_ptr<FlatIAABB>, unsigned> _objectNodeIndexMap;
		...
		void updateLeaf(unsigned leafNodeIndex, const FlatAABB& newAaab);
	...
	public:
		...
		void insertObject(const std::shared_ptr<FlatIAABB>& object);
		void removeObject(const std::shared_ptr<FlatIAABB>& object);
		void updateObject(const std::shared_ptr<FlatIAABB>& object);
		std::forward_list<std::shared_ptr<FlatIAABB>> queryOverlaps(const std::shared_ptr<FlatIAABB>& object) const;
	};
}